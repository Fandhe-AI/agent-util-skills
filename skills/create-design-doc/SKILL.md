...
`create-pitch-deck` が先に実行され PPTX が生成済みかどうかを **Glob で探索する**（既定パス
`_/pitch-deck/*.pptx` を直書きで決め打ちしない。ユーザーが `--output` を変更している場合が
あるため）。見つかった場合は Step 8 の相互整合チェックで参照する。見つからない場合は
「create-pitch-deck 未実行」として扱う。
